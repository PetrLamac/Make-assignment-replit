...
export default function AIAnalysisNode({ data }: NodeProps) {
  ...
  return (
    <div className="relative">
      ...
      <Card
        className={`w-64 p-4 bg-white border-2 shadow-md hover-elevate ${
          isAnalyzing
            ? "border-processing"
            : result?.status === "ok"
              ? "border-success"
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          >
            <Sparkles
              className={`w-4 h-4 ${isAnalyzing ? "text-processing animate-pulse" : "text-secondary"}`}
            />
          </div>
          <div className="flex-1">
            ...
            <div className="text-xs text-muted-foreground">
              Analyzes error messages using AI vision
            </div>
          </div>
          {getStatusBadge()}
        </div>
